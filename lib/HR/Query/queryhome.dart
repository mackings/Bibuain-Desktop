...
class _QueryhomeState extends State<Queryhome> {
  ...
  // For older dates, show the full date
    ...
    DateFormat dateFormatter = DateFormat("d MMM, ${timeFormatter.pattern}"); // e.g., 12 July, 2:01 PM
  ...
  if (selectedStaff != null) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        fetchMessages(name, responseBody['data']['_id']); // Assuming response has query ID
      } else {
...
Expanded(
      ...
      ? Padding(
        ...
        child: Column(
            children: [
              ...
              Row(
                children: [
                  Text("${selectedStaff!['name']}",style: GoogleFonts.montserrat(fontWeight: FontWeight.w500),),
                ],
              ),
              SizedBox(height: 20,),
  ...
  child: ConstrainedBox(
    constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width * 0.6), // Limit width to 60% of the screen
    child: Container(
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: TextStyle(fontSize: 16), // Adjust font size for the message
          ),
          SizedBox(height: 5), // Add some space between message and timestamp
          Text(
            formattedTime,
            style: TextStyle(
              color: Colors.grey[600], // Subtle color for timestamp
                    ...
                    : Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.only(top: 35.0), // Adjust padding as needed
...
IconButton(
  ...
  onPressed: () async {
    ...
    if (messageController.text.isNotEmpty && selectedStaff != null) {
      String name = selectedStaff!['name'];

      // Get the query ID from the selected staff's queries
      String queryId = selectedStaff!['queries'] != null && selectedStaff!['queries'].isNotEmpty
          ? selectedStaff!['queries'][0]['_id'] // Adjust based on your data structure
          : "";
      ...
      if (queryId.isNotEmpty) {
        try {
          ...
          final response = await http.post(
            Uri.parse('https://b-backend-xe8q.onrender.com/query/hrreply'),
            headers: {"Content-Type": "application/json"},
            body: body,
          );
          ...
          if (response.statusCode == 200) {
            final responseBody = json.decode(response.body);
            print('Message sent successfully: ${responseBody['data']}');
            ...
            setState(() {
              messages.add({
                'message': messageController.text,
                'senderRole': 'hr',
                'timestamp': DateTime.now().toIso8601String(), // Adding current timestamp
              });
            });
        ...
        print('No valid query ID found for sending message.');
      }
    }
  },
),
      ...
      : Center(
          child: Text("Select a staff member to view queries",style: GoogleFonts.montserrat(
                                      color: Colors.black,
                                      fontSize: 18,
                                    ),),
        ),
